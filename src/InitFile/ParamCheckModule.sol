...
contract ParamCheckModule is IModule {
    ...
    event setParamRange(address project, bytes4 sig, uint64 index, uint256 min, uint256 max); // 设置参数范围事件
    event removeParamRange(address project, bytes4 sig, uint64 index, uint256 min, uint256 max); // 移除参数范围事件
    ...
    function setInfo(bytes memory data) external override check {
        (bytes4 sig, address project, uint64 index, uint256 min, uint256 max) =
    ...
    function removeInfo(bytes memory data) external override check {
        (bytes4 sig, address project, uint64 index) = abi.decode(data, (bytes4, address, uint64));
        emit removeParamRange(
            project, sig, index, paramRanges[project][sig][index].min, paramRanges[project][sig][index].max
        );
    ...
    function detect(address project, string[] memory args, bytes memory data) external override returns (bool) {
        // 权限控制
        require(msg.sender == router, "ParamModule:detect only router can call");
        // 参数长度不匹配
