...
contract FireWallRouterTest is Test {
    ...
    function setUp() public {
        ...
        registry.addModule(address(paramModule), param_manager, "param detect", true);
        // 部署黑名单模块
        authModule = new AuthModule(address(router), address(registry));
        registry.addModule(address(authModule), auth_manager, "black detect", true);
        // 部署测试合约
        ...
        vm.stopPrank();
        bytes memory data = abi.encode(testContract.test_attack.selector, address(testContract), 0, 0, 100);
        vm.prank(deployer);
        registry.updataModuleInfo(address(paramModule), data);
        bytes memory auth_data = abi.encode(testContract.test_attack.selector, address(testContract), black);
        vm.prank(deployer);
