...
contract FireWallRouterTest is Test {
    ...
    function setUp() public {
        ...
        proxyForRegistry = new ProxyForRegistry(address(registry), admin, InitData_Registry);
        ...
        proxyForRouter = new ProxyForRouter(address(router), admin, InitData_Router);
    ...
    function test_RouterProxyUpgrade() public { 
        ...
        // 暂停
        vm.prank(deployer);
        bytes memory pauseData = abi.encodeWithSignature("pauseProject(address)", address(testContract));
    ...
    function test_RegistryProxyUpgrade() public{
        ...
        vm.prank(black);
        bytes memory unpauseData = abi.encodeWithSignature("unpauseProject(address)", address(testContract));
        ...
        vm.prank(admin);
        proxyForRegistry.upgradeToAndCall(address(RegistryV2), InitData_Registry);
        console.log("upgrade finished!!!");
        // 升级之后再次暂停
        vm.prank(deployer);
        bytes memory pauseData2 = abi.encodeWithSignature("pauseProject(address)", address(testContract));
        proxyForRegistry.CallOn(pauseData2);
        // 普通用户无法再解锁
        vm.prank(black);
        bytes memory unpauseData2 = abi.encodeWithSignature("unpauseProject(address)", address(testContract));
